...
ssh $PEER "nohup ${PEER_VALINOR_HOME}/build/mburst_marker -- -p ${PEER_INTERFACE_ID} -c ${PEER_VALINOR_HOME}/server.json > foo.out 2> foo.err < /dev/null &"
sudo timeout -s INT 15 ./build/mburst_marker -- -p $LOCAL_INTERFACE_ID -c client.json 
