...
/* protects @tcp_conns */
static rte_spinlock_t tcp_lock;


// static LIST_HEAD(tcp_conns, tcpconn);
...
int trans_table_add(struct trans_entry *entry)
{
	...
	uint32_t hash;
	if(entry->match == TRANS_MATCH_3TUPLE)
		hash = trans_hash_3tuple(IPPROTO_TCP, entry->laddr);
	else
		hash = trans_hash_5tuple(IPPROTO_TCP, entry->laddr, entry->raddr);
	ret = rte_hash_add_key_data(transport_hash[core_id], &hash, entry);
	if (ret < 0){
		log_error("Failed to add trans entry to the hash.");
		return ret;
	}
	return 0;
}
...
int trans_table_remove(struct trans_entry *entry)
{
	int ret;
	unsigned int core_id = 0;
	uint32_t hash = trans_hash_5tuple(IPPROTO_TCP, entry->laddr, entry->raddr);
	ret = rte_hash_del_key(transport_hash[core_id], entry);
	if (ret < 0){
		log_error("Failed to remove trans entry from the hash (%d)", ret);
		return ret;
	}
	return 0;
}
...
static void tcp_handle_timeouts(tcpconn_t *c, uint64_t now)
{
	...
	if (!c->tx_exclusive && !list_empty(&c->txq)) {
		struct tcp_mbuf *m = list_top(&c->txq, struct tcp_mbuf, link);
	...
	if (do_ack)
		tcp_tx_ack(c);
	if (do_retransmit)
		tcp_retransmit(c);  // TODO: instead of another thread. Is it correct?
}
...
void tcp_conn_ack(tcpconn_t *c, struct list_head *freeq)
{
	...
	while (true) {
		...
		list_pop(&c->txq, struct tcp_mbuf, link);
...
tcpconn_t *tcp_conn_alloc(void)
{
	...
	log_debug("TCP allocating connection ...");

	c = rte_malloc(NULL, sizeof(*c), 0);
	if (!c)
		return NULL;

	/* general fields */
	memset(&c->pcb, 0, sizeof(c->pcb));
	rte_spinlock_init(&c->lock);
	kref_init(&c->ref);
	c->err = 0;
	...
	c->pcb.iss = rand_crc32c(0x87654321); /* TODO: not enough */
...
int tcp_conn_attach(tcpconn_t *c, struct netaddr laddr, struct netaddr raddr)
{
	...
	rte_spinlock_lock(&tcp_lock);

	list_add_tail(&tcp_conns, &c->global_link);
	rte_spinlock_unlock(&tcp_lock);

	return 0;
}

static void tcp_conn_release(tcpconn_t *c)
{
	rte_spinlock_lock(&tcp_lock);
	list_del_from(&tcp_conns, &c->global_link);
	rte_spinlock_unlock(&tcp_lock);

	if (c->tx_pending->mbuf)
		rte_pktmbuf_free(c->tx_pending->mbuf);
...
void tcp_conn_release_ref(struct kref *r)
{
	...
	if (c->pcb.state != TCP_STATE_CLOSED)
		log_fatal("BUG: pcb state is not CLOSED");
	tcp_conn_destroy(c);
}
...
static void tcp_queue_recv(struct trans_entry *e, struct rte_mbuf *m)
{
	tcpqueue_t *q = container_of(e, tcpqueue_t, e);
	tcpconn_t *c;

	if(q == NULL)
		log_warn("Could not retrieve tcpqueue object");

	/* make sure the connection queue isn't full */
	rte_spinlock_lock(&q->l);
	if (unlikely(q->backlog == 0 || q->shutdown)) {
		rte_spinlock_unlock(&q->l);
		goto done;
	}
	q->backlog--;
	rte_spinlock_unlock(&q->l);
 ...
 *
 * Returns 0 if no new connection; 1 if successful, otherwise -EPIPE if the listen queue was closed.
 */
int tcp_accept(tcpqueue_t *q, tcpconn_t **c_out)
{
	...
	c = list_top(&q->conns, tcpconn_t, queue_link);
	if(c->pcb.state != TCP_STATE_ESTABLISHED)
	{
		// SYN received but handshake is not yet complete!
		rte_spinlock_unlock(&q->l);
		return 0;
	}
...
static ssize_t tcp_read_wait(tcpconn_t *c, size_t len,
			     struct list_head *q, struct tcp_mbuf **mout)
{
	struct tcp_mbuf *m;
	size_t readlen = 0;

	log_debug("TCP_read w1");
	*mout = NULL;
	log_debug("TCP_read w2 %lu", c->ack_delayed);
	rte_spinlock_lock(&c->lock);
	log_debug("TCP_read w3");
	...
	while (!c->rx_closed && (c->rx_exclusive || list_empty(&c->rxq))){ // TODO:DEBUG HERE
		rte_spinlock_unlock(&c->lock);
		return 0;
	}
	...
	while (readlen < len) {
		m = list_top(&c->rxq, struct tcp_mbuf, link);
		if (!m)
			break;
		...
		if (len - readlen < mbuf_length(m)) {
			log_debug("TCP Read Wait: acquired payload of size: %u", mbuf_length(m));
			c->rx_exclusive = true;
			*mout = m;
			readlen = len;
			break;
...
static void tcp_read_finish(tcpconn_t *c, struct tcp_mbuf *m)
{
	if (!m)
		return;
	rte_spinlock_lock(&c->lock);
...
ssize_t tcp_read(tcpconn_t *c, void *buf, size_t len)
{
	...
	if(c == NULL)
	{
		log_warn("We should iterate over all ready conns. return 0 if no conn is ready");
		return 0;
	}
	...
	if (ret <= 0)
	{
		// log_warn("read returned error status %d", ret);
		return ret;
	}
	/* copy the data from the buffers */
	while (true) {
		struct tcp_mbuf *cur = list_pop(&q, struct tcp_mbuf, link);
		if (!cur)
			break;
		ret = mbuf_pull_data(cur, &data_ptr);
		memcpy(temp, data_ptr, ret);
		memcpy(pos, data_ptr, mbuf_length(cur));  // TODO: copy data between pos and cur
		pos += mbuf_length(cur);
	...
	if (m) {
		log_warn("This code path may lead to invalid payload pulls. Must be fixed later");
		size_t cpylen = len - (uintptr_t)pos + (uintptr_t)buf;
		ret = mbuf_pull_data(m, &data_ptr);
		memcpy(pos, data_ptr, cpylen); // TODO: probably not correct. must be checked later
		m->seg_seq += cpylen;
	}
...
ssize_t tcp_readv(tcpconn_t *c, const struct iovec *iov, int iovcnt)
{
	...
	while (true) {
		...
		//	       mbuf_pull(m, cpylen), cpylen);  // TODO: read packet data?
...
{
	// log_debug("TCP Write waiting");
	rte_spinlock_lock(&c->lock);
	// log_debug("TCP Write wait acquired lock");
	/* block until there is an actionable event */
	if (!c->tx_closed &&
	       (c->pcb.state < TCP_STATE_ESTABLISHED || c->tx_exclusive ||
		wraps_lte(c->pcb.snd_una + c->pcb.snd_wnd, c->pcb.snd_nxt))) {
		// waitq_wait(&c->tx_wq, &c->lock);
		// log_debug("TCP Write wait not writable");
		rte_spinlock_unlock(&c->lock);
		return -1;
	}
	...
	if (c->tx_closed) {
		rte_spinlock_unlock(&c->lock);
		log_warn("TCP Write wait: socket closed");
...
static int tcp_conn_shutdown_tx(tcpconn_t *c)
{
	...
	if (c->pcb.state == TCP_STATE_ESTABLISHED)
		...
		log_warn("tcp con shutdown state failed");
...
void tcp_abort(tcpconn_t *c)
{
	...
	for (i = 0; i < 10; i++) {
		if (tcp_tx_raw_rst(l, r, snd_nxt) == 0)
			return;
		rte_delay_us_block(10);  // TODO: make sure it works! alternative: rte_delay_us_sleep
	}
