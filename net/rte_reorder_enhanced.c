...
rte_reorder_insert(struct rte_reorder_buffer *b, struct rte_mbuf *mbuf)
{
	...
	log_debug("REORDER: offset= %u, seq=%u, minseq=%u", offset, mbuf->seqn, b->min_seqn);
...
unsigned int
rte_reorder_pull(struct rte_reorder_buffer *b, struct rte_mbuf **mbufs,
		unsigned max_mbufs)
{
	unsigned int drain_cnt = 0;

	log_debug("b seq = %u (%d)", b->min_seqn, b->is_initialized);
	...
	log_debug("b draincnt = %u", drain_cnt);
...
int
rte_reorder_update(struct rte_reorder_buffer *b, unsigned int expected_sequence_number)
{
	b->min_seqn = expected_sequence_number;
	b->order_buf.head = (b->order_buf.head + 1) & b->order_buf.mask;
	b->is_initialized = 1;
	return 0;
}